...
/* Only types actually generated by hooks */
...
export function App() {
  ...
  // Real stats from server (DB totals, not local pagination counts)
  ...
  // Fetch stats from analytics overview (real DB totals)
  useEffect(() => {
  ...
  // Paginated fetch for specific project
  ...
  // Incremental paginated loading — fetch only types that still have data
  ...
  // Reset + automatic fetch when project filter changes
  useEffect(() => {
  ...
  // Scroll + highlight after search navigation
  useEffect(() => {
    if (highlightObsId === null) return;
    const targetId = `obs-${highlightObsId}`;
    // Attempt scroll with retry (data might not be in the DOM yet)
    let attempts = 0;
    const tryScroll = () => {
  ...
  return (
    <div className="h-screen overflow-hidden flex bg-surface-0">
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        {/* Header (only above content, not on sidebar) */}
        <Header
