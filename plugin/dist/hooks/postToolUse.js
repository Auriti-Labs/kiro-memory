...
runHook("postToolUse", async (input) => {
  ...
  if (readOnlyTools.includes(input.tool_name)) {
    ...
    const sdk2 = createTotalRecall({ project: project2, skipMigrations: true });
    try {
...
function buildReadObservation(toolName, toolInput, files) {
  ...
  if (isSearch) {
    ...
    const pathHint = path ? ` in ${basename2(path)}` : "";
    return {
...
function buildObservation(toolName, toolInput, toolResponse, files) {
  if (!toolInput) {
  ...
  switch (toolName) {
    ...
    case "Bash": {
      ...
      if (stdout) {
        ...
        const outputLines = lines.slice(-3).join("; ");
...
function buildContent(toolName, toolInput, toolResponse) {
  let content = "";
  if (toolInput) {
...
function extractFiles(toolInput, toolResponse) {
  const files = [];
...
function extractConcepts(toolName, toolInput, files) {
  ...
  const codeContent = (toolInput?.new_string || toolInput?.content || "").substring(0, 2e3).toLowerCase();
  if (codeContent.length > 20) {
    extractConceptsFromCode(codeContent, concepts);
  }
  return [...concepts].slice(0, 5);
}
function extractConceptsFromCode(code, concepts) {
...
function generateSubtitle(type, toolName, toolInput, files) {
