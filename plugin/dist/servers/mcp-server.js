...
Avvia il worker con: cd <kiro-memory-dir> && npm run worker:start`
  ...
  const safeMsg = msg.includes("Worker") ? "Errore di comunicazione con il Worker" : "Errore interno nell'elaborazione della richiesta";
  return {
  ...
  server.registerTool(
    "search",
    {
      ...
      inputSchema: {
        ...
        type: external_exports.string().optional().describe("Filtra per tipo di osservazione: file-write, command, research, tool-use, constraint, decision, heuristic, rejected (opzionale)"),
        limit: external_exports.number().optional().default(20).describe("Numero massimo di risultati (default: 20)")
      }
    },
    async (args) => {
      try {
...
`;
  ...
  server.registerTool(
    "timeline",
    {
      description: "Mostra il contesto cronologico attorno a una specifica osservazione. Utile per capire cosa \xE8 successo prima e dopo un evento.",
      inputSchema: {
        anchor: external_exports.number().describe("ID dell'osservazione come punto di riferimento"),
        depth_before: external_exports.number().optional().default(5).describe("Numero di osservazioni prima (default: 5)"),
        depth_after: external_exports.number().optional().default(5).describe("Numero di osservazioni dopo (default: 5)")
      }
    },
    async (args) => {
      try {
...
`;
  ...
  server.registerTool(
    "get_observations",
    {
      ...
      inputSchema: {
        ids: external_exports.array(external_exports.number()).describe("Array di ID osservazioni da recuperare")
      }
    },
    async (args) => {
      try {
...
`;
  ...
  server.registerTool(
    "get_context",
    {
      ...
      inputSchema: {
        project: external_exports.string().describe("Nome del progetto")
      }
    },
    async (args) => {
      try {
...
`;
  ...
  server.registerTool(
    "semantic_search",
    {
      ...
      inputSchema: {
        query: external_exports.string().describe("Query in linguaggio naturale per la ricerca semantica"),
        project: external_exports.string().optional().describe("Filtra per nome progetto (opzionale)"),
        limit: external_exports.number().optional().default(10).describe("Numero massimo di risultati (default: 10)")
      }
    },
    async (args) => {
      try {
...
`;
        output += `- **Disponibile**: ${result.available ? "s\xEC" : "no"}
`;
...
`;
  ...
  server.registerTool(
    "store_knowledge",
    {
      description: "Salva conoscenza strutturata: constraint (regole), decision (scelte architetturali), heuristic (preferenze soft), o rejected (soluzioni scartate). La conoscenza \xE8 valorizzata nei ranking di ricerca.",
      inputSchema: {
        knowledge_type: external_exports.enum(["constraint", "decision", "heuristic", "rejected"]).describe("Tipo di conoscenza: constraint (regole hard/soft), decision (scelte architetturali con alternative), heuristic (preferenze soft), rejected (soluzioni scartate con motivo)"),
        title: external_exports.string().describe("Titolo breve e descrittivo per la voce di conoscenza"),
        content: external_exports.string().describe("Contenuto dettagliato che spiega la conoscenza"),
        project: external_exports.string().describe("Nome del progetto (obbligatorio)"),
        severity: external_exports.enum(["hard", "soft"]).optional().describe("Per i constraint: hard (non deve mai essere violato) o soft (preferibile seguire)"),
        alternatives: external_exports.array(external_exports.string()).optional().describe("Per decision/rejected: opzioni alternative considerate"),
        reason: external_exports.string().optional().describe("Per decision/rejected: perch\xE9 questa scelta \xE8 stata fatta o scartata"),
        context: external_exports.string().optional().describe("Per heuristic: quando questa preferenza si applica"),
        confidence: external_exports.enum(["high", "medium", "low"]).optional().describe("Per heuristic: livello di confidenza"),
        concepts: external_exports.array(external_exports.string()).optional().describe("Concetti/tag correlati (opzionale)"),
        files: external_exports.array(external_exports.string()).optional().describe("File correlati (opzionale)")
      }
    },
    async (args) => {
      try {
...
- **Titolo**: ${args.title}`
  ...
  server.registerTool(
    "resume_session",
    {
      ...
      inputSchema: {
        project: external_exports.string().optional().describe("Nome del progetto (opzionale, usa il progetto auto-rilevato dall'ambiente)"),
        session_id: external_exports.number().optional().describe("ID sessione specifica da riprendere (opzionale, usa l'ultimo checkpoint per il progetto)")
      }
    },
    async (args) => {
      try {
        ...
        if (!checkpoint || checkpoint.error) {
          return {
            content: [{
              type: "text",
              text: "Nessun checkpoint trovato. Non c'\xE8 nessuna sessione precedente da riprendere per questo progetto."
            }]
          };
        }
        const parts = [
...
_Checkpoint creato: ${checkpoint.created_at}_`);
  ...
  server.registerTool(
    "save_memory",
    {
      ...
      inputSchema: {
        project: external_exports.string().describe("Nome del progetto (obbligatorio)"),
        title: external_exports.string().describe("Titolo breve e descrittivo per il ricordo"),
        content: external_exports.string().describe("Contenuto completo del ricordo da salvare"),
        type: external_exports.string().optional().default("research").describe("Tipo di osservazione: research, file-write, command, ecc. (default: research)"),
        concepts: external_exports.array(external_exports.string()).optional().describe("Concetti/tag correlati (opzionale)")
      }
    },
    async (args) => {
...
- **Titolo**: ${args.title}`
  ...
  server.registerTool(
    "generate_report",
    {
      description: "Genera un report di attivit\xE0 per un progetto. Restituisce un riepilogo markdown con osservazioni, sessioni, apprendimenti, task completati e file hotspot per il periodo specificato.",
      inputSchema: {
        project: external_exports.string().optional().describe("Nome del progetto (opzionale, usa il progetto auto-rilevato)"),
        period: external_exports.enum(["weekly", "monthly"]).optional().default("weekly").describe('Periodo: "weekly" (default) o "monthly"')
      }
    },
    async (args) => {
      try {
        ...
        const d = result;
        const parts = [
          `# Report Attivit\xE0 \u2014 ${d.period?.label || period}`,
